...
public class PurpurTemplateProvider implements TemplateProvider {
    private final List<String> versions = new ArrayList<>();
    private final Map<String, List<String>> builds = new HashMap<>();

    public PurpurTemplateProvider() {
        try {
    ...
    public OCTemplate getTemplate(@NotNull String name) {
        if (!name.startsWith("purpur")) return null;
        ...
        return new PurpurTemplate(version, build);
    }

    private void loadBuilds(@NotNull String version) {
        if (builds.containsKey(version)) return;
        List<String> builds = new ArrayList<>();
        String url = "https://api.purpurmc.org/v2/purpur/" + version;
        try {
            HttpURLConnection con = (HttpURLConnection) new URL(url).openConnection();
            con.setConnectTimeout(5000);
            con.setReadTimeout(5000);
            ...
            if (con.getResponseCode() == 200) {
                InputStreamReader reader = new InputStreamReader(con.getInputStream());
                JsonObject json = new JsonStreamParser(reader).next().getAsJsonObject();
                JsonObject buildsObject = json.getAsJsonObject("builds");
                JsonArray all = buildsObject.getAsJsonArray("all");
                if (all.isEmpty()) return;
                for (JsonElement build : all) {
                    builds.add(build.getAsString());
                }
            }
