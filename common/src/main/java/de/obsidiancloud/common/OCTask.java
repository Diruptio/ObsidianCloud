 ...
 * @param linkToProxies The proxies to link the servers to (or {@code null} if the servers should be
 *     linked to any proxies)
...
public record OCTask(
    ...
    public static @NotNull OCTask fromFile(@NotNull Path file) {
        ...
        if (config.contains("min-amount")) {
            ...
            throw new IllegalArgumentException(
                    "\"min-amount\" (int) is missing in task file: " + file.getFileName());
        }
